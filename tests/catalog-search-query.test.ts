...
test('catalog text search joins expanded terms with OR', () => {
  ...
  assert.equal(
    ...
    [
      '(',
      "(coalesce(title, '') || ' ' || coalesce(author, '') || ' ' || coalesce(book_category, '')) ILIKE $3",
      'OR',
      "(coalesce(title, '') || ' ' || coalesce(author, '') || ' ' || coalesce(book_category, '')) ILIKE $4",
      'OR',
      "(coalesce(title, '') || ' ' || coalesce(author, '') || ' ' || coalesce(book_category, '')) ILIKE $5",
      ')',
    ].join(' '),
  );
